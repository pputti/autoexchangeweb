...
const offerModel = require("../models/trading");
...
exports.watch = (req, res, next) => {
  ...
  model
    .findOne({ title: userId, author: tradeId })
    .then((trade) => {
      console.log(trade);
      if (!trade) {
        const watchModelList = new watchModel({
          userID: userId,
          tradeId: tradeId,
        });
        watchModelList
...
exports.show = (req, res, next) => {
  ...
  model
    ...
    .then((trade) => {
      if (trade) {
        watchModel
          .findOne({ userID: user, tradeId: id })
          .then((tradeItem) => {
            let watched = false;
            if (tradeItem) {
              watched = true;
...
exports.startTrading = (req, res, next) => {
  ...
  model
    .findByIdAndUpdate(
      id,
      { Status: "Offer Pending", tradeOffered: true },
      { useFindAndModify: false, runValidators: true }
    )
    .then((trade) => {
      console.log("User ID:");
      console.log(user);
      newOfferItem = new offerModel({
        Name: trade.title,
        Status: "Offer Pending",
        Category: trade.category,
        tradeOfferedBy: user,
      });
      console.log(newOfferItem);
...
exports.onTrading = (req, res, next) => {
  ...
  Promise.all([
    ...
    offerModel.findOne({ tradeOfferedBy: user }),
  ])
    .then((results) => {
  ...
  model
    ...
    .then((trade) => {
      ...
      Promise.all([
        model.findOneAndUpdate(
          { title: name },
          { Status: "Available", tradeOffered: false },
          { useFindAndModify: false, runValidators: true }
        ),
        offerModel.findOneAndDelete({ Name: name }),
      ])
